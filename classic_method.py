...
def action_start(img, result_list, hcom):
        ...
        for obj in result_list:
            ...
            if 'pour water' in task_type:
                if 'bottle' in obj:
                    ...
                    # 移动机械臂到瓶子位置并抓取
                    Button8Click(x_r_bottle, y_r_bottle, z_r_bottle+200, 80.0, 0.0, 2500,hcom)
                    time.sleep(2)
                    Button8Click(x_r_bottle, y_r_bottle, z_r_bottle, 80.0, 0.0, 2500,hcom)
                    time.sleep(2)
                    Button8Click(x_r_bottle, y_r_bottle, z_r_bottle, 80.0, 0.0, 1800,hcom)
                    time.sleep(2)
                    # 将瓶子抬高
                    Button8Click(x_r_bottle, y_r_bottle, z_r_bottle + 100, 80.0, 0.0, 1800,hcom)
                    time.sleep(2)
                    ...
                    if cup_recognized:
                        # 移动到杯子位置
                        Button8Click(x_r_cup, y_r_cup+110, z_r_bottle + 100, 80.0, 0.0, 1800,hcom)
                        time.sleep(2)
                        if rbot.dx_get > 0:
                    ...
                    if bottle_captured:
                        # 如果已抓取瓶子，移动到杯子位置并倒水
                        Button8Click(x_r_cup, y_r_cup, z_r_cup, 80.0, 0.0, 1800,hcom)
                        if rbot.dx_get > 0:
                            print(f"接收到数据: X={rbot.XP}, Y={rbot.YP}, Z={rbot.ZP}")
                            print(
                                f"接收到数据: a0={rbot.a0}, a1={rbot.a1}, a2={rbot.a2},w0={rbot.w0},w1={rbot.w1},aw={rbot.aw}")
...
def detect2(hcom, custom_list, custom=False):
    ...
    if not custom:
        ...
        obj_dict = {'person': 0, 'bicycle': 1, 'car': 2, 'motorcycle': 3, 'airplane': 4, 'bus': 5, 'train': 6,
                    ...
                    'bed': 59, 'dining table': 60, 'toilet': 61, 'tv': 62, 'laptop': 63, 'mouse': 64, 'remote': 65,
                    'keyboard': 66, 'cell phone': 67, 'microwave': 68, 'oven': 69, 'toaster': 70, 'sink': 71,
                    'refrigerator': 72, 'book': 73, 'clock': 74, 'vase': 75, 'scissors': 76, 'teddy bear': 77,
                    'hair drier': 78, 'toothbrush': 79}
        desired_classes = [obj_dict[obj] for obj in custom_list]
    '''
    ...
    while True:
        ...
        for left_obj in left_objects:
            ...
            candidates = [(i, ro) for i, ro in enumerate(right_objects) if
                          ro[0] == left_cls and i not in used_right_indices]
            if len(candidates) == 0:
        ...
        if key == ord('a'):
            ...
            if user_input.strip() == 'action':
                ...
                action_start(base_left_frame, all_detect_result,hcom)
